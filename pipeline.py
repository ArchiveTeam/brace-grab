...
LUA_SHA1 = get_hash(os.path.join(CWD, 'brace.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--warc-header", "brace-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("brace-user: %(item_name)s"),
        ]
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="brace"),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
