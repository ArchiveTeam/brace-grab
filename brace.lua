...
wget.callbacks.download_child_p = function(urlpos, parent, depth, start_url_parsed, iri, verdict, reason)
  ...
  if item_type == "site" and (downloaded[url] ~= true and addedtolist[url] ~= true) then
    if string.match(url, "http[s]?://"..item_value.."%.brace%.io/") then
      return verdict
    elseif html == 0 then
      return verdict
    else
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if item_type == "site" then
    if string.match(url, "http{s]?://"..item_value.."%.brace%.io/sitemap%.xml") then
      html = read_file(file)
      for newurl in string.gmatch(html, "<loc>([^<]+</loc>") do
        if string.match(newurl, "http[s]?://"..item_value.."%.brace%.io/") then
          check(newurl)
        end
      end
    end
